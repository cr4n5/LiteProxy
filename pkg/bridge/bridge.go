...
func (b *Bridge) handleConnection(ctx context.Context, conn *quic.Conn) {
	...
	case "server":
		log.Infof("(SERVER) Remote Addr %s: connected with client ID (%s)", conn.RemoteAddr().String(), hm.ClientID)
		for {
			newStream, err := conn.AcceptStream(ctx)
			if err != nil {
				log.Errorf("(SERVER) Remote Addr %s: failed to accept server stream: %v", conn.RemoteAddr().String(), err)
				return
			}
			log.Infof("(SERVER) Remote Addr %s: new stream accepted for client ID (%s)", conn.RemoteAddr().String(), hm.ClientID)
	...
	case "client":
		...
		if !ok {
			log.Errorf("(CLIENT) Remote Addr %s: client ID (%s) already connected", conn.RemoteAddr().String(), hm.ClientID)
			_ = conn.CloseWithError(0, "client ID already connected")
			return
		}
		log.Infof("(CLIENT) Remote Addr %s: client ID (%s) connected", conn.RemoteAddr().String(), hm.ClientID)

		// wait for disconnection
		<-conn.Context().Done()
		b.RemoveClient(hm.ClientID)
		log.Infof("(CLIENT) Remote Addr %s: client ID (%s) disconnected", conn.RemoteAddr().String(), hm.ClientID)
...
func (b *Bridge) handleServerStream(ctx context.Context, hm *lib.HandshakeMessage, stream *quic.Stream) {
	...
	if err != nil {
		log.Debugf("(SERVER) stream: piping between server and client ID (%s) ended: %v", hm.ClientID, common.TranslateStreamError(err))
		stream.CancelWrite(common.TranslateErrorCode(err))
		return
	}
	log.Debugf("(SERVER) stream: piping between server and client ID (%s) ended normally", hm.ClientID)
}
